...
**Applied 2026-09-14 to the dev sandbox `xhoqkhwsnqfwhjsffybs` only** — 16 + 11 columns and 9 indexes verified through `information_schema`. The test project `qsjrazcbtpmubclkevwi` answers `tenant/user not found` on both poolers (paused or deleted), so it could not be used. **Production (`qgfyfbdccjnibdhhvnsr`) does NOT have it yet** and must, before this branch reaches `dev` — `dev` deploys the production backend. Until then the Stay endpoints 500 there; nothing else is affected, since no other query reads these tables.
