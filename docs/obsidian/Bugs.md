...
**Cause:** the flag was carried only in `redirectUrlComplete`, on the assumption (stated in `ClerkOAuthCallbackPage.tsx`'s header) that `<AuthenticateWithRedirectCallback>` reads that value back off the sign-in attempt. It does not. clerk-js's `handleRedirectCallback` (read in `@clerk/clerk-js@5.128.0`, the version `@clerk/clerk-react@5.61.9` loads from the CDN — no version pin) builds its destination as `new RedirectUrls(options, props)` — the component's props and the provider options, **no search params, nothing from the attempt**. A brand-new Google account always finishes on `/sign-in/sso-callback` (Clerk has to transfer the sign-in to a sign-up client-side), so it landed on the prop fallback `/auth/callback` with no flag; `AuthCallbackPage` saw a plain `NO_STAYO_ACCOUNT` and took the rejection path. The same loss applies to `HostelLeadModal`'s `/lead-signup/callback` for any flow that finishes on the SSO callback.

**Fix:** the destination also travels on the SSO callback URL (`/sign-in/sso-callback?after=…`, `lib/auth/ssoCallbackDestination.ts`), and the callback page passes it as `signInForceRedirectUrl`/`signUpForceRedirectUrl`. Only a same-origin path is honoured (`//host`, `/\host`, schemes and control characters fall back to `/auth/callback`), since the value arrives in a URL anyone can craft. 11 node-only tests on the pure helper; the wiring into the two components is not covered by the node-only suite (`CLAUDE.md`).

**Not verified:** no live Google round trip. It assumes Clerk's FAPI preserves an existing query string on `redirectUrl` when redirecting back — expected, but unobserved. It also assumes the Clerk instance requires nothing beyond Google's own fields at sign-up; if it does (`missing_requirements`), clerk-js sends the person to the Account Portal's `/continue` step instead, which this fix does not address. Related: [[Frontend]], [[Changelog]].
...
**Root cause — the server sent ImageKit one form field per byte.** Commit `3f6b5793` (2026-09-10) changed both `POST /api/tenants/activate/photo` and `/activate/documents` from `file: buffer.toString("base64")` to `file: buffer`, on the belief that the SDK "accepts Buffer directly". `@imagekit/nodejs` v7 does not: its form encoder recognises File/Blob, Response, async iterables and strings, and a Node `Buffer` falls through to the generic *object* branch, which appends `file[0]`, `file[1]`, … for every byte. Reproduced: a 200KB image became **204,802 form fields and a 19MB body with no `file` part**; ImageKit rejects that with a 400 after the whole inflated request has been sent — which is where the waiting went. The SDK error (`"400 …"`) was passed through as the route's message. Every other upload in the codebase still used base64, so only onboarding broke. Backend builds run with `ignoreBuildErrors`, so the type mismatch never surfaced.

**Root cause — the client compressed too late, or not at all.** The same commit added in-browser compression, but `handlePhotoChange` checked the *original* against the 2MB photo limit first, refusing every ordinary 3-8MB camera photo before compression ran; if the eager upload failed, the retry at submit sent the uncompressed original. The compressor had no `onerror`, so an undecodable image left the spinner running forever, and it labelled its output `image/webp` even where the browser (Safari < 16) silently produced a PNG — often larger than the original and over the limit.

**Fix.** `lib/imagekit-uploadable.ts` turns binary input into a real `File` (`toFile`), used by both routes and by the `lib/imagekit.ts` wrapper for every caller, so a raw Buffer can no longer reach the SDK; `tests/imagekit-uploadable.test.ts` captures the actual multipart body with an injected `fetch`. On the client, `compressImage.ts` + the pure, tested `uploadImagePolicy.ts` resize and re-encode on the device **before** any limit is checked — photo 1280px / target 400KB, documents 2200px (small print stays legible) / target 1.2MB, always JPEG, decode timeout, never hangs — then check against the server's own limits with a message the tenant can act on. The compressed file is what the submit-time retry sends. Upload requests time out after 90s instead of never. Measured in Chrome: a 7.9MB 12MP photo → 322KB profile photo (134ms) or 926KB Aadhaar at 2200px. A live upload through the fixed wrapper reached ImageKit and served back (then deleted).

**Note:** `features/hostel-drilldown/marketing/mediaUpload.ts` has its own `compressImage` for listing photos, with a single size policy. Onboarding's needs (per-kind limits, format conversion, a quality ladder, a decode timeout) differ, so the two were not merged.

**See:** [[APIs]], [[Frontend]], [[Changelog]], [[Business-Rules]]
...
**The download was the lesser half.** The same `include: { hostel: true }` appears twice more in `[batch_id]/confirm/route.ts` — the GET preview *and the POST that creates the tenants*. Confirm threw before doing anything, so against a live database the import had never been able to run. It was found only because the owner reached for a different button first.

**Root cause — the third instance of one failure mode.** `prisma` is exported as `any`, so a wrong relation name type-checks. Every test in this tree mocks the client, and a mock answers to whatever name it is handed — the fixtures in `bulk-import-chunked-confirm` and `bulk-import-issue-persistence` supplied `hostel`, so the suite was actively holding the wrong name in place. Compiler blind, tests complicit, review reading plausible English. Previously: `select: { profile: … }` for `profiles` ([[Bugs#2026-09-11 — Seven bulk-import columns were read, stored, and then ignored (fixed)|found in review]]), and `hostel` here.

**Fix.** The name, in both routes, and the fixtures that were pinning it. `tests/bulk-import-query-shapes.test.ts` now parses every route under `app/api/bulk-import` and checks each `include`/`select` key against `Prisma.dmmf` — the generated client's own datamodel, with no mock in between. It is anchored to each call's argument object so a nested select is checked against its own model, allows Prisma's `_count`-style pseudo-fields, and carries a vacuity check so a scanner that silently stops finding anything fails instead of passing. Mutation-tested on both an `include` and a `select`.

**Also fixed.** The workbook route had no error handling, so any failure became an opaque 500 and the screen could only offer "try again in a moment" — advice that was never going to work. It now logs the cause and returns a message that says the import is safe and the owner can carry on from what is on screen.

**Lesson.** A mocked client cannot verify a name it is only ever given. Anything that must match the schema has to be checked against the schema.

**See:** [[APIs]], [[Changelog]], [[Backend]]

## 2026-09-11 — A correct sheet was blocked by a date the owner never typed (fixed)

**Symptom.** An owner uploaded one tenant. Every date in the sheet was right — the row's `05/01/2026` parsed, stored as `2026-01-05`, and even earned the historical-joining-date warning — and the import was blocked anyway by *"2026-09-11 isn't a full date"*. `2026-09-11` was that day's date: the **default** joining date, generated by our own code, for rows that leave the column blank. This row did not leave it blank.

**Root cause — a fallback validated as though it were input.** The default was re-checked inside the per-row loop, unconditionally, and its failure was pushed as a `BLOCKER` against every row with `field: "joining_date"`. Three things were wrong at once: it was raised for rows that never touch the default; it was blamed on a cell the owner had filled in correctly; and its `PICK_DATE` fix pointed at that same correct cell, so editing and re-checking raised it again. An unfixable error on a valid import, and on a forty-row sheet it would have been forty of them.

**Why a date we generate could fail to parse at all.** `formatImportDate(new Date())` built a string and `parseImportDate` immediately read it back — a round trip with a failure mode and no purpose. The generated default is now a `Date` from the start and never becomes text that has to be re-read, so that path cannot fail.

**The same look-alike trap, for real cells.** A zero-width space, a left-to-right mark, an en dash in place of a hyphen, fullwidth digits from an IME, a non-breaking space — each renders as an ordinary date and each was rejected. The owner reads `2026-09-11` in the cell and is told it isn't a date: an error with no visible cause and therefore no fix. `parseImportDate` now normalises these before matching; every substitution maps a character to the one it was already imitating, so no date changes meaning.

**Why it hid.** Every test supplied a joining date on the row *and* a parseable default, so the branch that fires only when the two disagree was never taken. The defect needed a valid sheet to show itself, which is the opposite of where tests look.

**Fix.** The default is parsed once, outside the loop, and can only stop a row that actually falls back to it. Pinned by the owner's exact row, and mutation-tested — restoring the unconditional check fails the new test.
...
**Fix.** Account eligibility became the first rule, evaluated ahead of the tenancy rules for exactly that reason, and the profile lookup now selects `role` alongside `id`. The deep guard still stands as a backstop but now throws the structured `TenancyEligibilityError` the routes already know how to serialise, so the submit-time 409 and the debounced pre-submit check render the same card with the same words. `POST /api/leads/[id]/convert-to-invitation` gained the same serialisation, which it had never had. **The rule itself is [[Decisions#ADR-162|ADR-162]]'s, not this fix's:** it is hostel-scoped, so only the asking owner's own account (or an admin) is refused — an owner of a *different* hostel may be a tenant. See [[Decisions#ADR-164|ADR-164]], [[Business-Rules]], [[APIs]].
## 2026-09-01 — Lead/tenant/invitation identity guards: a missing tenant check, a crashing self-invite, and an over-broad owner/tenant block (fixed)

**Found:** during an audit of the owner→lead→invitation→tenant flow requested specifically to check mobile-as-primary-identity, duplicate-lead, self-invite, and owner/tenant-relationship rules. Not reported by a user — found by tracing the code against the intended business rules.

**Bug 1 — lead creation never checked the `tenants` table at all.** `admissions-service.ts`'s `createDirectLead` and `createLead` deduped only against other `visitor_leads` rows (by `hostel_id` + `student_phone`). A phone number that already had an `ACTIVE` or `INVITED` tenancy at that exact hostel could still be captured as a brand-new lead there, with no check against `tenants` whatsoever. **Fix:** both functions now call `tenancyEligibilityService.hasLiveTenancyAtHostel(phone, hostelId)` before the existing lead-dedup lookup and refuse if it matches. See [[Business-Rules]], [[Decisions#ADR-162|ADR-162]].

**Bug 2 — an owner inviting their own email (different phone) crashed with a raw 500.** Self-invite was blocked only as a side effect of `owner-managed-tenancy-service.ts`'s phone-collision check throwing `ROLE_MISMATCH` when the phone matched an existing non-TENANT profile — which caught the phone case but not the email-only case. Inviting with the owner's own email and an unrelated phone instead hit `profile.create()`'s unhandled `P2002` on `profile.email`'s unique constraint, surfacing as an unhandled server error rather than a validation message. **Fix:** `tenant-invitation-lifecycle-service.ts`'s `createInvitation` now explicitly compares the invited contact against the owner's own registered phone/email immediately after loading the owner's profile, before any write, and throws a clean `VALIDATION_ERROR`.

**Bug 3 — the owner/tenant-relationship check was global-role-scoped, not hostel-scoped, and silently blocked a rule the product wants allowed.** `tenancy-claim-service.ts` and `owner-managed-tenancy-service.ts` refused to bind **any** non-`TENANT`-role profile to a tenancy. That accidentally enforced "an owner cannot become their own hostel's tenant" but, as a side effect neither test nor code comment called out, also refused "an owner of Hostel A may become a tenant of Hostel B" — a combination the product explicitly wants allowed. Root cause: `profile.role` is one global field: `OWNER`, `TENANT`, or `ADMIN`, never both, and the check compared against that field alone instead of against the specific hostel being joined. **Fix:** both checks now compare the matched profile's id against *this tenancy's actual hostel owner* rather than against role alone — refusing only when they're the same hostel's owner. **Residual, deliberately unaddressed:** login/session role is still derived from the same global `profile.role`, so a profile that becomes both an owner (of A) and a tenant (of B) still authenticates as `OWNER` and cannot reach the tenant portal for B yet — see [[TODO]] and [[Decisions#ADR-162|ADR-162]].
...
### A blocked subscription would have 500'd instead of 402/409, the moment enforcement turned on

- **Found:** 2026-09-10, auditing billing error handling ahead of ever enabling `PLATFORM_BILLING_ENFORCED` ([[Decisions#ADR-187|ADR-187]] Phase 6.3). Not yet triggered in production — the flag has been off since Phase 3, so `assertOwnerSubscriptionActive` / `plan-capacity-service.assertCanActivate` have never actually thrown past this code.
- **Area:** [[Backend]] — ~40 owner platform-management mutation routes (`app/api/{tenants,hostels,rooms,floors,allocations,announcements,hostel-events,expenses,food,utility-status,owner}/**`) plus the tenant-activation entry routes (`tenants/invite`, `tenants/[id]/reactivate`, `tenants/owner/reactivation-requests/[id]/decision`).
- **Symptom (would-be):** the moment `PLATFORM_BILLING_ENFORCED=true`, a `PENDING_PAYMENT`/`PAUSED` owner hitting any of these routes, or an owner at their plan's tenant cap, would get a bare `500 Internal Server Error` with no code the frontend could recognise — instead of the intended `402 SUBSCRIPTION_INACTIVE` / `409 SUBSCRIPTION_CAPACITY_REACHED` the guard actually throws.
- **Root cause:** `assertOwnerSubscriptionActive` / `assertCanActivate` throw a `SubscriptionError` carrying its own `.status`/`.code`, but every one of these routes' own `catch (error) { ... }` reads only `error.message` — via a bare `apiError(error.message)` (default status 500), a raw `Response.json({..., status: 500})`, or a `message.split(":")`-prefix parser that expects `"CODE: text"` and has nothing to match against a human sentence like "Your Stayo subscription is awaiting payment." Each route's own error handling was written before Phase 3 added the subscription guard to it, and nothing re-checked it once the guard was inserted.
- **Fix:** new `billingErrorResponse(error)` in `subscription-http.ts` — returns the correctly-mapped response for a `SubscriptionError`, `null` for anything else. Inserted as the first line of the relevant `catch` block in all ~43 affected route files (mechanical, verified 1:1 against each guard call site), so every other error in that route is handled exactly as before. `app/api/tenants/activate/route.ts` already special-cased `SUBSCRIPTION_INACTIVE`/`SUBSCRIPTION_CAPACITY_REACHED` correctly (Phase 3) and needed no change; the platform-admin subscription/payment routes already used the equivalent `subscriptionErrorResponse` uniformly.
- **Lesson:** a guard added to N routes is only as good as the Nth route's *existing* catch block — inserting a throw into code that was never written to expect that specific error type is a silent trap that a default-off feature flag can hide for a long time. See [[Changelog]].

### The owner Tenant Detail Documents tab showed no KYC documents at all, only "Rental agreement — Awaiting signature"

- **Found:** 2026-09-11, owner reported the Documents tab on the tenant detail page (e.g. tenant "Rithwik") never showed uploaded Aadhaar/College-ID documents or any Approve/Reject affordance — only the rental agreement card, permanently stuck at "Awaiting signature" even for signed agreements.
- **Area:** [[Frontend]] — `apps/frontend/src/features/tenants/api/index.js` (`tenantService.getDocuments`), consumed by `apps/frontend/src/features/owner-tenants/hooks/useTenantDetail.ts` and rendered by `apps/frontend/src/features/owner-tenants/pages/TenantDetailPage.tsx`.
- **Symptom:** the entire KYC-review feature — `DocumentReviewCard` approve/reject UI, `documentGroups.ts`'s `kyc`/`reviewRequests` grouping, the agreement's "View & download" preview — was fully built and wired up, but always rendered empty/disabled because it never received any documents.
- **Root cause:** `GET /api/tenants/[id]/documents` returns `{ documents, required_documents }`, but `tenantService.getDocuments()` unwrapped this to a bare array (`data?.documents ?? []`), discarding `required_documents`. Its sole caller, `useTenantDetail.ts`, reads `documentsResult.documents` / `documentsResult.required_documents` — both `undefined` on a plain array — so `presentDocs`/`requiredTypes` were always `[]`. The `agreementDoc` lookup (also sourced from the same always-empty list) was always `null`, so the agreement's `previewable` flag was always false regardless of actual signature status.
- **Fix:** `getDocuments()` now returns the `{ documents, required_documents }` object shape the backend already sends and the consumer already expected (with a defensive `Array.isArray` fallback for the plain-array case). No backend, hook, or component changes were needed — this was a single data-shape bug at the API-wrapper boundary, not a missing feature. See [[Changelog]].
- **Lesson:** a consumer's own error-fallback shape (`useTenantDetail.ts`'s `.catch(() => ({ documents: [], required_documents: [] }))`) already documented the intended contract; the wrapper it called had silently drifted from it. When a `.catch` fallback's shape looks deliberate, check it against the happy path before assuming the happy path is right.

### Creating the first Stayo-listed hostel failed with "Argument `id` is missing"

- **Found:** 2026-09-14, creating a Stayo listing from the admin dashboard (`POST /api/platform-admin/platform-listings`) in production. Every attempt failed; no platform listing could ever have been created, because the very first one has to create the sentinel owner.
- **Area:** [[Backend]] — `apps/backend/src/services/marketing/platform-owner.ts` (`getOrCreatePlatformOwnerProfile`), the "Stayo Platform" sentinel profile that owns unclaimed platform listings so `hostels.owner_id` can stay NOT NULL.
- **Symptom:** `Invalid prisma.profile.create() invocation … Argument id is missing.` — returned verbatim to the admin UI as a 500.
- **Root cause:** `profiles.id` is `@id @db.Uuid` with **no default** (checked in `schema.prisma` and on the production database) — real accounts reuse their Supabase auth user id, so every other `profile.create` supplies one explicitly. The sentinel has no auth user and its create omitted `id`. Nothing caught it before runtime: `lib/db.ts` exports `prisma: any`, so Prisma payloads are never type-checked, and `next.config` sets `ignoreBuildErrors: true`.
- **Fix:** the sentinel mints its own id with `crypto.randomUUID()` — the same pattern the admin-invite route and the tenant-invitation services already use. The payload is now also `satisfies Prisma.profileUncheckedCreateInput`, which restores a compile-time check on this one call: with the id removed, `tsc` reports the same "Property 'id' is missing". A DB-backed regression test (`tests/platform-owner-profile.test.ts`) covers first-use creation and idempotency; it **has not run**, because the test Supabase project was paused (`INACTIVE`) at the time. See [[Changelog]].
- **Lesson:** with `prisma` typed as `any`, a required column with no default is only enforced at runtime. Any new `profile.create` must supply `id`; wrapping a payload in `satisfies Prisma.<model>UncheckedCreateInput` gets the type check back without having to retype the whole client.

### The dog's head was clipped on the login card, because a centred dialog makes no room for what leans on it

- **Found:** 2026-09-14, from a screenshot of `/discover` with the Sign Up tab open in a ~975px-tall window.
- **Area:** [[Frontend]] — `shared/ui-patterns/LoginModal.tsx` and `shared/ui/brand/mascot/`.
- **Symptom:** the top of the Stayo dog was cut off by the top of the browser window, and the card scrolled inside itself with the native scrollbar crossing its rounded corner.
