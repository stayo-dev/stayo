...
Applied via a hand-written migration (`prisma/migrations/20260808130000_add_food_polls/migration.sql`), executed directly rather than through `prisma migrate deploy`/`db push` — both are currently blocked in this dev environment by **pre-existing, unrelated** issues found during this work: a failed migration already recorded against `20260501000000_add_late_fee` (blocks `migrate deploy` entirely until resolved), and `schema.prisma` no longer declaring two `tenants` columns (`reservation_policy`, `minimum_reservation_deposit`, removed from the schema in `9597421` without a matching migration) that still hold 390 real rows in the live dev DB (`db push` would silently drop them). Neither is related to Food Polls and neither was touched — flagged here since the next schema change in this repo will hit the same wall.
