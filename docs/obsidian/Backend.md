...
| Tenant analytics/migration/self-service portal | `lib/services/` — `tenant-analytics-service.ts`, `tenant-migration-service.ts` (**a deprecated stub, not bulk import** — its import logic was removed in the auth-hardening audit; it survives only so `tests/auth-hardening-security.test.ts` and `tests/activation-enforcement-coverage.test.ts` can assert it no longer creates tenants), `tenant-profile-portal-service.ts` |
| Bulk tenant import | `lib/services/bulk-import/` (split 2026-09-10 from a single 638-line `bulk-import-validation-service.ts`, which remains as a re-export shim so route imports did not change): `types.ts`, `workbook-parser.ts` (workbook → rows; every owner-fixable failure carries the `VALIDATION_ERROR:` prefix), `dates.ts` (DD/MM/YYYY-first; ISO and Excel serials still accepted), `identity.ts`, `room-resolution.ts` (nearest-room suggestions, `1O1` → `101`), `issues.ts` (the owner-facing `RowIssue` catalogue — see [[APIs]]), `sanitize-row.ts` (the **one** allowlist of what is persisted per row — upload and revalidate used to carry divergent copies), `file-type.ts`, `validation-service.ts`. Execution is in `app/api/bulk-import/[batch_id]/confirm/route.ts`, which delegates each row to `tenant-invitation-lifecycle-service.createInvitation` — the same path as the single-tenant wizard. See [[Bugs]] 2026-09-10 and [[Business-Rules]] |
| Hostels/properties/portfolio/dashboards | `lib/services/` — `property-service.ts` (931 lines), `hostel-policy-service.ts`, `room-capacity-service.ts`, `portfolio-service.ts`, `portfolio-performance-service.ts`, `dashboard-service.ts` (1659 lines), `hostel-activity-feed-service.ts` (the per-hostel activity timeline, extracted from `app/api/owner/activity-logs/route.ts` 2026-09-14 — [[Decisions#ADR-198|ADR-198]]; mostly *reconstruction* from live tables, and its two audit-table reads scope to the hostel by **different** mechanisms — read the comments before changing either) |
...
- `profiles.auth_user_id` (nullable, unique) + `auth_linked_at` link a profile to `auth.users`. Linking is just-in-time (`lib/auth/supabase-identity.ts`'s `ensureSupabaseIdentity()`), on every successful password login — not a backfill/forced reset. **That, and signups born linked, are the only ways a profile gains an identity ([[Decisions#ADR-200|ADR-200]]):** `resolveSupabaseSession()` resolves a token by `auth_user_id` alone and refuses one no profile is linked to (`NO_STAYO_ACCOUNT`, logged as `AUTH_SUPABASE_UNLINKED_REJECTED`) — it no longer falls back to an email match, which let anyone holding a Supabase session for an address take over the profile carrying it. `npm run reconcile:supabase-identities` (dry-run by default, `--apply` to write) reports/repairs linkage state by cross-referencing `profiles` against `auth.users` by both `id` and `email`.
