...
- [ ] **Nothing behind a login has been exercised.** Public browse *was* checked in a browser on 2026-09-13 (split screen at `/`, Discover listing, all seven formerly-`410` endpoints answering correctly). But **no enquiry has been submitted end to end**, no listing taken through owner edit → admin review → live, and no move-out walked. **This remains the largest gap** — these surfaces were unrouted for ten days and are now reachable by the public.
...
- [x] ~~**Submit the three generation-2 rent templates to Meta**~~ — done, and the rollout mechanism has been removed. All four (`stayo_rent_due_reminder`, `stayo_rent_due_today`, `stayo_rent_overdue_reminder`, `stayo_payment_receipt`) are APPROVED in `en`, verified against the live WABA on 2026-09-14. The env-var switch turned out to be the outage: its v1 fallback names had never existed at Meta, so every send failed `132001` while the owner UI reported success. Name, language and parameter order now live in code together ([[Decisions#ADR-196|ADR-196]]); the `WHATSAPP_*_TEMPLATE` variables are inert and can be deleted from Vercel. **Confirmed delivered 2026-09-14:** resident `READ`, guardian `DELIVERED`, payment link opened from the message. See [[Bugs]].
...
- [x] ~~**The whole product is off-brand typographically.**~~ **Done 2026-09-09.** `apps/frontend/src/styles/theme.css`'s unscoped `:root` now carries the brand pack's Manrope + Inter and the Stayo product palette, the retired theme's values are deleted from the source, and `npm run check:brand` fails the build if any of them reappear. See [[Decisions#ADR-172|ADR-172]].
- [ ] **The frontend's `.dark` block and its 128 `dark:` utilities are dead code.** `styles/theme.css` still defines a `.dark` token set in shadcn's default greyscale `oklch(…)` values — no brand colours in it — and no code ever puts a `dark` class on the document, so none of it renders. It survived [[Decisions#ADR-172|ADR-172]] because it is inert rather than off-brand, and deleting it is a separate decision from retiring the old identity. Either build a real Stayo dark theme from the pack (Charcoal `#2F2F2F` is the pack's dark surface) or remove the block and the utilities. **why:** right now the repo looks like it supports dark mode and does not, so the next person to add a `dark:` variant will believe it does something — **related:** [[Decisions#ADR-172|ADR-172]], [[Frontend]].
- [ ] **The rebuilt orientation spotlight has not been watched running.** `spotlightPlacement.ts` is unit-tested (10 cases), but the caption measures itself in a second layout pass and is parked off-screen for that first paint — on a real device that should be checked for a visible flicker, along with the arrow's alignment against the bottom-nav tabs, which is the tightest clamp case. **why:** placement maths passing in node says nothing about what a phone shows — **related:** [[Decisions#ADR-173|ADR-173]], [[Features]].
