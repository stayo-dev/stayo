...
- [x] **Fold all six crons into `vercel.json` and retire `.github/workflows/backend-cron.yml`.** Done 2026-09-08, [[Decisions#ADR-173|ADR-173]]. `keep-warm.yml` (5-minute `/api/health` ping) stays on GitHub — a sub-daily schedule fails Hobby deployment — and is now the only GitHub Actions schedule in the repo; it can move too on Pro.
