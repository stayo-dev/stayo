...
- [ ] **Make the "+ Invite" refusal actionable.** When an owner invites someone who already has an owner-managed tenancy *at their own hostel*, the 409 says "This person is already a tenant at your hostel X" and stops. It should offer the claim link instead ([[Decisions#ADR-150|ADR-150]]) — that is what the owner was trying to do. Left alone because that path runs through the invite wizard `main` rewrote on 2026-08-29. **why:** it is the exact dead end that produced the bug report — **related:** [[Decisions#ADR-150|ADR-150]].
