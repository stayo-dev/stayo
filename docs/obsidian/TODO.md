...
- **Rent changes don't reprice obligations that have no `agreement_id`.** `applyRentChangeInTx` selects unpaid obligations by `agreement_id`, but obligations generated for hostels with `agreement_required = false` are written with `agreement_id = null` (1 of 23 `RENT` obligations on the live database as of 2026-08-26). For those tenants a rent change updates `agreement.contract_rent` and `tenants.monthly_rent` — so future months are right — but already-generated unpaid obligations keep the old amount. Either backfill the link at generation time or widen the repricing selector to fall back to `tenant_id` + `rent_month`. See [[Bugs]] (Change rent was impossible for any hostel that doesn't require agreements).
