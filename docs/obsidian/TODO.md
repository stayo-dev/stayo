...
<<<<<<< HEAD
- [ ] **The invite backfill ignores the hostel's configured rent due day.** `tenantInvitationLifecycleService.createInvitation` calls `initializeOnboardingFinancials` without a `dueDay`, so every reconstructed month falls back to the hardcoded default of 5 regardless of what the hostel set. `/api/owners/invitations/settlement-preview` deliberately mirrors that default rather than the real setting, because a preview must match what the writer creates — fixing the writer should fix both at once. **why:** an owner who set a due day of 1 or 10 sees reconstructed months dated the 5th — **related:** [[Decisions#ADR-141|ADR-141]], [[APIs]].
- [ ] **The prior-history settlement is unverified against a database.** `priorTenancySettlement.settleInTx` is covered only through its pure input (`resolvePriorTenancyPayment`, 13 tests); the transaction itself creates a tenant and so cannot run until the test database gains `tenants.access_mode`. **why:** it writes settled financial records against a real tenant's ledger — **related:** [[Decisions#ADR-141|ADR-141]], [[Bugs]].
...
=======
>>>>>>> parent of 98aad6c (feat(invite): record what an existing tenant has already paid)
