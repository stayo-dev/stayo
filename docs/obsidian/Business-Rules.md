...
- **Only the fields the wizard actually collects are absorbed** — an explicit whitelist (`date_of_birth`, `gender`, `guardian_name`/`_phone`/`_relation`, `photo_url`, `college_name`, `course`, `year_of_study`, `branch`, `roll_number`, `office_name`, `office_location`, `job_role`), not the whole saved row. `profile_type` is excluded because `tenants.profile_type` **defaults to `"STUDENT"`**, and `permanent_address`, `personal_email`, `nationality` and `pan_number` are excluded because this form never asks for them. Onboarding may only teach the portable profile things the tenant actually confirmed on this form: these fields are shared across every hostel the person has ever stayed at, so absorbing the raw row let hostel B flip a working professional back to STUDENT and let an owner's invite-time guess overwrite the person's own record.
...
- **Server, `saveAccount()`:** `isAlreadyVerified` requires both that the profile/tenant already carries a verified flag (`mobile_verified || phone_verified`) **and** that the verified number matches the number being submitted. An OTP is demanded only when that's false. **The match is on the last ten digits, not the raw string** — self-signup stores `919876543210` while `normalizeIndianPhone()` yields `+919876543210`, and `buildKnown()` (which decides whether the UI even shows an OTP box) already compared digits. Two formats across that seam meant the form hid the OTP field and the server then demanded a code the UI could not collect.
- **"Change" is intent, not a change.** The frontend passes `phoneEdited && typedDigits !== knownDigits`, so a tenant who taps Change and retypes the same verified number is not forced through an OTP the server would ignore.
