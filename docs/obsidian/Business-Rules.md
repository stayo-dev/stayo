...
### Tenant in-app notifications (2026-08-29)

Added because the tenant app's notification bell was purely decorative — it fired a static "No new alerts" toast and never read real data. **Files:** `app/api/service-requests/[id]/status/route.ts`, `app/api/announcements/route.ts`, `lib/services/notification-service.ts`. Frontend: `apps/frontend/src/features/notifications/hooks/useTenantNotifications.ts`, `apps/frontend/src/platforms/tenant/pages/TenantNotificationsPage.tsx`.

- **Two triggers write to the shared `notifications` table** (the same table/service the owner-side Alerts bell already reads for admin broadcasts — see [[Database]] for the schema):
  1. **Service-request ticket update.** When an owner PATCHes a `tenant_service_requests` status (assign/progress/resolve/reject, optionally with a reply `note` — e.g. "here's the WiFi password"), the tenant gets a notification (`type: "service_request"`) carrying `metadata.requestId` so the frontend can open that exact ticket rather than parsing an id out of text. Message is the owner's note if given, else a generic "Status updated to {status}" line.
  2. **Owner announcement.** When an owner posts a `hostel_announcements` row, every **live** tenant of that hostel (`LIVE_TENANCY_STATUSES`, i.e. `INVITED`/`ACTIVE`) with a linked account gets a notification (`type: "announcement"`), fanned out via `Promise.allSettled` so one failed write never blocks the announcement itself.
- **An owner-managed tenant with no account (`tenants.profile_id` null) gets nothing** — same precedent as the WhatsApp-reminder "no in-app surface for someone with no account to open" rule above.
- **In-app only, no WhatsApp/email** — a deliberate scope decision; these two triggers do not send anything outside the app.
- **Superseded 2026-08-29 (same day):** the "deliberately not built" chat feature below this line was the original scope call — the owner's "Chat" button (`AlertsRequestsPage.tsx`) was a `soon()` stub at the time. A follow-up same-day request explicitly asked for a real two-way chat on that button, so it was built — see [[#Two-way ticket chat (2026-08-29)|Two-way ticket chat]] below. This entry is left as a record of the original, narrower decision.

### Two-way ticket chat (2026-08-29)

Turns the owner's "Chat" button on a service-request row (`features/owner-alerts/pages/AlertsRequestsPage.tsx`, previously a `soon()`/"Coming soon" stub) into a real two-way thread, reusing the same `tenant_service_request_events` timeline the tenant-notifications work above already reads — not a new chat/message table. **Files:** `lib/services/service-request-notifications.ts` (new, shared), `app/api/service-requests/[id]/messages/route.ts` (new, owner), `app/api/tenants/me/service-requests/[id]/messages/route.ts` (new, tenant), `app/api/service-requests/[id]/status/route.ts` (refactored to use the shared helper, behavior unchanged). Frontend: `features/owner-alerts/components/ServiceRequestChatSheet.tsx` (new), tenant's `serviceRequestDetailConfig.ts`'s "Updates" section becomes a `messages`-kind chat thread (`overlays/Section.tsx`, `overlays/types.ts`).

- **A chat message never changes ticket status.** `tenant_service_request_events.status` is now nullable (`ServiceRequestStatus?`, migration `20260829130000_service_request_events_status_nullable`) — a pure chat message writes `status: null`; a real status transition (via the existing `PATCH .../status` route, unchanged) keeps writing a value. Both message routes create an event and **never** call `tenant_service_requests.update()` — there is no code path from a chat send to a status change.
- **Exactly one notification per sent message, to the other participant, carrying `metadata.requestId`.** Both message routes create one event, then call the shared `notifyServiceRequestParticipant(profileId, request, message)` helper exactly once — no retry loop, no fan-out beyond the single counterpart. Owner → tenant: skipped if the tenant has no account (same precedent as above). Tenant → owner: always sent — `hostels.owner_id` **is** the owner's `profile_id` directly (same fact the `announcements` route relies on), so no extra lookup is needed; the message is prefixed with the tenant's name via `resolveTenantName()` (`lib/tenants/tenant-identity.ts`) since the owner's Alerts inbox has no other per-row sender context.
- **The timeline is the chat.** A `status: null` event renders as a chat bubble (aligned by `actor_role`); a `status`-set event renders as a small centered pill (unchanged visual role, just no longer the only kind of entry). This is why no new table was introduced — the tenant side already rendered every event's note as a list before this change, just read-only.
- **The loop closes on both sides.** A tenant notification's tap already opened the exact ticket (`metadata.requestId` → `/tenant/complaints` with `openTicketId`, from the tenant-notifications work above) — a chat message reuses that unchanged. On the owner side, `/owner/alerts`'s "Admin messages"/Announcements tab (already reading unread `notifications` rows) now passes `metadata` through, and tapping a row with `metadata.requestId` navigates to `/owner/alerts/requests` with `state: { openChatRequestId }`, which `AlertsRequestsPage.tsx` reads once on mount to open that ticket's chat sheet directly — mirroring the tenant-side mount-effect pattern exactly. Accepted minor label mismatch: a tenant's chat reply shows up under the owner's "Announcements" category page, since that page generically renders any unread `notifications` row regardless of `type`.
- **`GET /api/service-requests` (owner queue) and `GET /api/service-requests/[id]/messages`** both now include each ticket's most recent event, enabling a `hasUnreadFromTenant()` helper (`features/hostel-content/api/serviceRequests.ts`) — "the last timeline entry was from the tenant" — for a future unread indicator; not yet wired into any row UI as of this change.
- **No read-receipts, no gating by ticket status** — chat works on `RESOLVED`/`REJECTED` tickets too, matching how `note` already behaved on the status route before this change.
- `notifications.metadata` (`Json?`) is new — see [[Database]].
...
4. **`RECEIPT` asks which payment, then attaches the PDF.** A resident half a year in has made six payments and the one they want is rarely the newest, so the command opens a picker of their most recent 10 (amount and date lead each row — what people actually remember). Asking is skipped where there is no real choice: **one** payment is sent straight away, **none** is answered plainly. The chosen payment's id rides in the payload (`CC:RECEIPT:<tenantId>:<paymentId>`) and is re-scoped to the tenant on arrival, so a payload that passed through the reader's handset cannot reach another resident's payment.
   - **The document, not a reference to it.** `ensureReceiptDocument` calls `receiptService.generatePdfBuffer(paymentId, { autoCreate: true })` — the canonical reuse-or-generate path, which creates the `receipts` row if missing, serves the cached PDF when `receipt_template_version` is current, and otherwise re-renders and re-uploads to ImageKit. The resulting `receipts.receipt_pdf_url` is public (the service already re-fetches it unauthenticated), which is what lets Meta attach it: Meta fetches `document.link` **server-side**, so the authenticated `/api/payments/[id]/receipt` route could never have been used. The filename carries the receipt number, not a UUID.
   - **A failed render is not silence.** If no URL can be produced, the reply names the payment so the reader can quote it to the hostel. If the document send itself fails, the URL is handed over as text.
   - Payment confirmations attach the same receipt, rendered once per payment rather than once per recipient.
5. **Instalment position excludes deposits and maintenance.** `PLAN` counts rent obligations only — folding a one-off security deposit into "3 of 12" produces a count a parent would reasonably act on and that is wrong. Tenants with no billing plan (month-to-month) get ordinal numbering with no total.
6. **Nothing prints "N/A".** `@/lib/format`'s date helpers return that string on null; `voice.ts::shortDate` returns `null` instead, and any sentence that needed the value is not written.
...
Rule 0 was added 2026-09-01 ([[Decisions#ADR-164|ADR-164]]) and is evaluated **first**, because such an account normally holds no tenancies at all — exactly the shape rules 1 and 2 wave through. **It is [[Decisions#ADR-162|ADR-162]]'s hostel-scoped rule, not a blanket one:** an `ADMIN` is never a tenant, and an `OWNER` may never be a tenant of the hostel they own — but an `OWNER` of a *different* hostel may rent a room like anyone else. The invite wizard only ever invites into the asking owner's own hostels, so "is this the asking owner's own account?" is exactly the hostel-scoped question, and it is answerable on the Tenant step, before any hostel has been chosen. It **fails closed** with no `invitingOwnerId`: an unknown owner is never treated as a different one. Supplying the account is opt-in (`EligibilityOptions.account`): a caller that already holds a tenant omits it and skips the rule rather than guessing.
...
**A refused account discloses nothing but whose it is (2026-09-01).** `PHONE_BELONGS_TO_NON_TENANT` carries `scope: OWN` only when the number is the *asking owner's own* — which is the common case, an owner typing their own number to test the form — and `OTHER` otherwise, which after the hostel scoping above means an admin account and nothing else. `hostelName`, `roomNumber` and `tenantId` are always `null`: no name, no hostel, not even whose account it is. The owner learns "this number can't be a tenant" and nothing further, which is what stops the refusal becoming an account-enumeration oracle over every owner and admin on the platform.
...
**The pre-submit exit this paragraph originally described is gone (2026-08-28, [[Decisions#ADR-136|ADR-136]]).** The wizard used to offer a second exit, "Just add to my records," gated by its own `isOwnerManagedValid` (requiring `!eligibilityConflict`, same as `isStep0Valid`/`isStep3Valid`) so it couldn't be used as a back door around the Send-invite block. That exit is removed outright, not merely re-gated — see "Owner-managed tenants" below for the replacement flow, which asks the same question *after* the invite is sent instead of before.

**An owner may be a tenant of a *different* hostel; never of their own (2026-09-01, [[Decisions#ADR-162|ADR-162]]).** Whether the matched profile is allowed through the checks above is not "is this profile role TENANT" but "does this profile own *this specific hostel*" — `owner-managed-tenancy-service.ts` compares the matched profile's id against this tenancy's actual `owner_id`, not against a blanket non-TENANT-role refusal. (Originally also enforced in `tenancy-claim-service.ts`; that file — and the phone-OTP claim flow it belonged to — was removed on 2026-09-01, see below.) An `OWNER` of Hostel A may claim/be adopted into a tenancy at Hostel B; an `OWNER` of Hostel A may never claim/be adopted into a tenancy at Hostel A itself. An `ADMIN` profile, or an `OWNER` who does own this exact hostel, is still refused. **Known limitation:** `profile.role` remains a single global field, and login session role is derived from it — so an owner who successfully becomes a different hostel's tenant still authenticates with an `OWNER` session today, and cannot yet reach that tenancy's portal via `resolveTenantScope`. This ADR only removed the incorrect rejection; making the resulting account usable from both sides is separate, tracked work — see [[TODO]].

## Lead creation cannot re-capture an existing tenant, and an owner cannot invite themselves (2026-09-01, [[Decisions#ADR-162|ADR-162]])

Two guards on the `visitor_leads` → `tenant_invitations` path, distinct from the `platform_leads` owner-acquisition-funnel rule above:
...
- A fresh invitation sets `acceptance_status = PENDING`, leaves `activation_completed_at` / `tenant_accepted_at` **null**, writes **no** `tenant_owner_attestations` row, and leaves the invitation row `PENDING` (not `SUPERSEDED`).
- **Only the tenant, personally, moves it to `ACCEPTED`** — `completeActivation`, reached by walking their own `/activate/:token` wizard, sets `ACCEPTED` + `tenant_accepted_at` + `activation_completed_at` and flips `access_mode` to `SELF_SERVE`. There is no owner path that does this: `adopt` / "Keep records myself" / `suppressInvitationNotification` are all removed.
- **While `PENDING`, the owner cannot fill tenant-only fields** — guardian details, ID/document verification, college/education, DOB/gender/photo/permanent address, `document_verified`. `updateTenant` hard-rejects them (`VALIDATION`), rather than creating change requests a login-less tenant can never approve. Owner-operable fields (name, invitation phones, terms via the amendment flow, `mobile_verified`, notes) are unaffected.
- **A `PENDING` tenancy that is never accepted is cleanly torn down.** Owner: `POST /api/tenants/[id]/cancel-invitation` (now accepts `ACTIVE`+`PENDING`, → `CANCELLED`). Automatic: a daily cron closes tenancies whose link expired more than `GRACE_DAYS` (default 7) ago and are not mid-activation (→ `EXPIRED`). Both free the room and **void only *future* unpaid `RENT`/`MAINTENANCE` obligations** (`rent_month` past the current period, no payment); past/current dues, every recorded payment, the security deposit and the ledger are preserved for settlement (`closeUnacceptedTenancy`).
- **Grandfathering:** every existing `OWNER_MANAGED` row keeps `acceptance_status = NOT_REQUIRED` and is untouched — the field-lock, the expiry sweep and the new invariants all ignore it; the activation predicates fall back to the old `invitationStatus`/`ownerAttested`/`activation_completed_at` proxies for it. No backfill.
- **The activation guards read `acceptance_status`, not `tenant.status === "ACTIVE"`.** `activation-entry.ts`'s `hasCompletedActivation` / `isAwaitingTenantOnboarding` gain an `acceptanceStatus` input that is authoritative when present; `resolveByToken`'s `ALREADY_ACTIVE` rung calls `hasCompletedActivation`. The two "no new stay until settled" readers (`tenancy-eligibility-service.wasActivated`, `residency-history-service.ever_moved_in`) now also count `status === "ACTIVE"` / `acceptance_status === "ACCEPTED"` as "was operationally live", since `activation_completed_at` is no longer stamped at invite.

## Owner-managed tenants — access is a second, independent axis (2026-08-27, Phase 1)

> **Amended by [[Decisions#ADR-165|ADR-165]] (2026-09-02):** the "Adopt" and "Keep the records myself meanwhile" transitions below, `tenant_owner_attestations` writes, and stamping `activation_completed_at` at invite/adopt time are all **removed** for new tenancies. `access_mode` and the two-axis model stay; what a new invitation produces is `ACTIVE` + `OWNER_MANAGED` + `acceptance_status = PENDING`, and the tenant's own activation is the only way to `SELF_SERVE`. The rules below describe grandfathered (`acceptance_status = NOT_REQUIRED`) rows.
...
**Identity is centralised on `profiles`, keyed by canonical phone (2026-08-28, [[Decisions#ADR-136|ADR-136]]) — supersedes the paragraph below as originally shipped.** As designed in Phase 1, `OWNER_MANAGED` meant `profile_id: NULL` — the person's name/phone lived only on `tenants.display_name`/`phone_1`, and no `profiles` row existed for them at all. That made an adopted tenancy invisible to every guard in this codebase that resolves a phone/email to a *profile* first (which is all of them — see the eligibility rule above and [[Bugs]] for the production incident this caused). **`OWNER_MANAGED` now means a `profiles` row that cannot sign in, never the absence of a `profiles` row.** `ownerManagedTenancyService.adopt` resolves one profile per human, keyed on canonical phone: reuse an existing profile matching that phone untouched (name/email/password/role left exactly as they are), or create a new one with `auth_user_id: null` and `password_hash: null`. That flag — not the presence or absence of `profile_id` — is the entire definition of "owner-managed" now. `display_name` remains a display-layer fallback (`resolveTenantName`), unchanged from Phase 1.

**Transitions:**

| Transition | Trigger | Effect |
|---|---|---|
| **Adopt** | Owner: "Keep records myself" on a quiet invite (`POST /api/tenants/[id]/adopt`) | `INVITED → ACTIVE`, `access_mode = OWNER_MANAGED`, a profile resolved-or-created and linked (see below), reservation converted to a room allocation, an attestation written. Rent generation begins on the next run. |
| **Keep the records myself meanwhile** *(renamed 2026-08-28, [[Decisions#ADR-136|ADR-136]])* | Owner, on the success screen right after sending an invite (`InviteDeliveryResult`) | Same `adopt` call as above, against the tenancy `inviteMutation` just created — no second tenancy, no suppressed-notification flag. **Replaces** the earlier pre-send "Just add to my records" exit, which created its own tenancy via `createInvitation({ suppressInvitationNotification: true })` before adopting it; that exit no longer exists. |
| **Activate** *(unified 2026-09-01, superseding Phase 2's claim flow, [[Decisions#ADR-163|ADR-163]])* | Tenant completes the same activation wizard every invitee uses (`/activate/:token`) | `OWNER_MANAGED → SELF_SERVE`, `completeActivation` (`tenant-invitation-lifecycle-service.ts`). Same `tenant_id`, same financial history. See "Claim flow removed" below and [[APIs]]. |

**Adopt does not cancel the invitation.** The superseded invitation stays resolvable, because a stale link the tenant clicks after adoption routes into the same activation screen (see below), not to an error.
...
- `OWNER_MANAGED` (any status) → must **never** hold a linked auth identity — checked as `profiles.auth_user_id IS NOT NULL` via the `profiles` relation. **Corrected 2026-08-28 ([[Decisions#ADR-136|ADR-136]]):** this line previously read "since an owner-managed tenant should have no `profiles` row at all" — that was true under Phase 1's original design but is no longer accurate. An owner-managed tenant now always *has* a `profiles` row (a login-less one, created or reused at adoption); what the check actually enforces, correctly, is that it never gains an `auth_user_id`. This is still the invariant that matters most: it is the one that would catch a Phase-2 claim bug leaving a tenant simultaneously "owner-managed" and able to log in as someone else.
...
**Phase 2 (2026-08-27) shipped a separate phone-OTP claim flow; removed 2026-09-01.** See "Claim flow removed" below for what replaced it. **Still not built:** Revoke (`SELF_SERVE → OWNER_MANAGED` for a tenant who stopped using the app). See [[TODO]].
...
**A charge that was raised by hand can be corrected or withdrawn — never erased** (2026-09-11, [[Decisions#ADR-186|ADR-186]]). Obligations stay immutable: correcting one raises a replacement and cancels the original; withdrawing one marks it `CANCELLED` with a reason the tenant reads. Both need the owner's password (`CANCEL_OBLIGATION` step-up) and a reason, and neither is possible once a payment has landed against the charge — that is a waiver, not a withdrawal. Rent and security deposit are excluded: they follow the tenancy, and changing them is the rent-change flow. The withdrawn charge stays on the tenant's timeline, struck through, showing the owner's reason.

**Upload limits, and where an image is shrunk.** Server-side: a profile photo is JPG/PNG/WEBP up to 2MB (`/activate/photo`), a KYC document adds PDF and allows 5MB (`/activate/documents`). Images are resized and re-encoded **on the device** before either limit is applied — a document keeps a 2200px longest edge so the owner can still read the small print, a profile photo 1280px (see [[Frontend]]). A PDF is never re-encoded, so an over-size PDF asks the tenant for a photo of the document instead.
...
**The fix instead: `resolveByToken` resolves that case itself.** When an invitation is `SUPERSEDED` and the tenancy is `OWNER_MANAGED` and the tenant has not yet been through onboarding (`isAwaitingTenantOnboarding`), `resolveByToken` (`tenant-invitation-lifecycle-service.ts`) falls through into the exact same success shape a never-superseded invitation returns, instead of throwing. A tenant clicking their original WhatsApp link — or a stale link from before the owner later adopted their tenancy — lands on the same blue activation wizard as every other invitee, pre-filled with whatever the owner already entered (room, rent, agreement). If the tenant has already completed onboarding, the link now correctly reports `ALREADY_ACTIVE` instead of the old `CLAIM_REQUIRED`.

**They walk the full wizard, not a shortcut.** Unlike the old claim flow — whose `confirm` pre-bound the profile, pre-verified the phone, and pre-wrote a `TenantPolicyAcceptance` before the tenant ever saw a wizard screen — an owner-managed tenant reaching activation this way goes through the same `ACCOUNT → RULES → PROFILE → AGREEMENT → ACTIVATE` sequence (see "Whether a tenant must sign an agreement" below) as anyone else. `profile_id` is already bound (from invite/adoption time), but `phone_verified`/`mobile_verified` and any rules acceptance are not, so `isAccountSetupComplete`/`computeState` correctly report those steps as not yet done.

**`completeActivation` now tolerates having no active reservation.** An owner-managed tenancy's reservation was already converted to a real allocation and released (`release_reason: "ADOPTED"`) at invite/adopt time — `completeActivation` used to hard-require an *active* reservation to exist and would throw `INVALID_TRANSITION` for this population. It now falls back to the tenancy's existing active allocation when no reservation is found, and only throws if neither exists.

**`access_mode` flips back to `SELF_SERVE` on real completion.** That write used to live only in the claim flow's `confirm`. It is now part of `completeActivation` itself, unconditionally — reached only after the tenant has genuinely walked the wizard themselves, which is definitionally self-serve regardless of how the tenancy started.

**The owner-facing share-link card survives, pointed at a real link.** `TenantDetailPage.tsx`'s "You're keeping these records" card (owner-managed tenants only) no longer builds a token-less `/claim` URL; it shares the tenant's actual `tenant.invitation.activationLink` (already resolved server-side to their most recent invitation regardless of status).
...
## Owner-acquisition funnel — one active lead per phone number (2026-08-31, [[Decisions#ADR-161|ADR-161]])
...
## Meet your host — how a listing presents the owner (2026-09-15)

See [[Decisions#ADR-200|ADR-200]], [[Features]], [[Database]]. Server rules live in `apps/backend/src/services/host-profile/` (`bio-rules.ts`, `host-stats.ts`); display rules in `apps/frontend/src/features/host-profile/model/hostCardModel.ts`. Both are pure and tested.

- **Full name, for hosts only.** A host card shows `profiles.name` trimmed with inner whitespace collapsed, on the button too ("Enquire with Shiva Prakash"). A **reviewer** is still first name + last initial; ADR-200 changed the host rule, not that one.
- **A bio carries no way to reach the owner off Stayo.** Refused, with a specific reason shown to the owner: a phone number (any run of 10+ digits once spaces, dashes, dots and brackets between digits are joined, so `+91 98765 43210` counts), an email, a link (`http(s)://`, `www.`, `wa.me`, or a bare domain on a common TLD with a label of 2+ characters, so "M.Com" is not a link), and an @handle of 3+ characters. **Admins are held to the same rule.**
- **Bio shape:** max 500 characters after normalising (CRLF → LF, trailing spaces dropped, 3+ newlines collapsed to a blank line, trimmed); empty means none. It renders in full with line breaks, never truncated.
- **Languages** come from a fixed list of 12 (Telugu, Hindi, English, Tamil, Kannada, Malayalam, Marathi, Urdu, Bengali, Gujarati, Punjabi, Odia), max 6, deduped, in the order picked. **Running since** is an integer from 1950 to the current year, or none.
- **Stats are earned on Stayo, counted live, never stored.** Rating = average of `PUBLISHED` reviews across all the owner's hostels, 1 decimal, shown only with ≥ 1 review. Residents = distinct `profile_id`s of `ACTIVE`/`FORMER_TENANT` tenancies on the owner's hostels (scoped through `hostels.owner_id`, because `tenants.owner_id` is nullable). Bulk-imported residents count. Fewer than 5 → omitted; 5–9 → exact; 10+ → floored to the ten and shown with "+". The residents figure is never combined with the self-reported running-since year. **Verified** = an active `owner_documents` row, `doc_type` `AADHAAR`/`PAN`, `status = VERIFIED`.
- **No zeros, no "new host".** A stat that is zero or unknown is left out of the public card. A first-year owner reads "On Stayo since September 2026". On the owner's own milestones strip the tile stays, with a placeholder ("Your first review will show here").
- **Moderation is live-on-save with an admin override.** An owner's edit is public straight away. Only an admin can set `bio_hidden`/`photo_hidden`; an owner write never touches them, and a hide survives owner edits until an admin lifts it. Hidden fields are dropped **on the server** (`getPublicHost`), never left for the client to hide. Admin writes are attributed and event-logged.
- **The host appears twice on a listing, and each placement has one job** (revised 2026-09-15, see [[Decisions#ADR-200|ADR-200]]). The **byline** near the top carries only photo, full name and one fact line — never the bio, the stats or the Enquire button, so the beds and the rent stay near the top of the page. The **full card**, after the reviews, carries everything else. Both come from the same `PublicHost` payload; neither recomputes anything.
- **Platform listings have no host.** A `PLATFORM_LISTED` hostel shows "Listed by Stayo", with no name, card, stats or button. **Exception (2026-09-21):** if the listing's reviewed content sets `basics.host_name`, the byline/card read "Hosted by <name>" · "Owner" — still no photo, stats or button, and still no live vacancy. See [[Changelog]].
- **The trust note makes no protection claim** and names no payment provider: "Pay and talk through Stayo, so there's a record of everything." See [[Decisions]] on published legal copy.
...
- **Maintenance is per row.** The sheet's Maintenance Charge and Maintenance Type columns are read for each tenant, falling back to the batch value on the upload form and then the hostel's billing defaults. A row whose type is `NONE` owes no maintenance whatever the default says. Sent as `maintenance_amount`, the key `createInvitation` reads.
...
- **Re-sending an invitation is not editing it.** A nudge re-sends the same link and restarts its 7-day clock (invitation and bed hold) without touching dues, so it is allowed for tenants with payments on record — every imported resident with paid history. Changing the offer's terms regenerates the dues and stays locked once payments exist. An invitation already closed by the expiry sweep cannot be nudged back to life.
- **A tenant's email is mandatory at onboarding, and proved with a code** ([[Decisions#ADR-183|ADR-183]]). The Identity screen collects it; the ACCOUNT step will not complete without a verification for that invitation and address. Skipped only for someone who already signs in with a real address of their own. An address that belongs to another account is refused ("use a different email"), never adopted. Until then the tenant has no email — owners see it blank, and the `@hms.temp` storage key is never displayed or emailed.
- **Back-rent needs the owner's explicit consent, and the review screen always asks for it.** Any joining date before today puts the batch behind a gate: `POST …/confirm` refuses it without `confirm_historical_join_dates: true`. Every row behind that gate carries an issue the owner can agree to — `RENT_BACKDATED` (within the cap, naming the first billed month and the month count) or `BACKFILL_CAPPED` (beyond it) — both `NEEDS_CHOICE` with an `ACKNOWLEDGE` fix. Unlike other choices these **hold the Import button** until agreed, and the client sends the confirmation only from that agreement, never because the server asked. The gate flag (`requires_historical_join_date_confirmation`) is derived from those issue codes via `needsHistoricalJoinDateConfirmation`, not from a warning string, so the gate and the control cannot drift apart again — they did, and every existing-resident import was refused with nothing on screen to change. See [[Bugs]].
- **Amount already paid** (parsed from an *Amount Already Paid* column; the current CSV template does not offer one yet) is settled inside the invitation transaction by `financialPaymentFacade.receivePayment`, **FIFO across every due** — back-rent, maintenance and the deposit obligation. A payment method is required: a row with an amount and no method is blocked at preview (`PAYMENT_METHOD_MISSING`) instead of failing at execution. **An amount above what the tenant owes is now blocked at preview too** (`OVERPAID`, a `BLOCKER` — the row cannot import until the amount or the joining date changes) — `financial-plan.ts` composes `buildInviteSettlementPreview`, the same pure planner behind the single-invite settlement panel, rather than recalculating dues.
- **"Paid Includes Deposit = No" is honoured.** The settlement is filtered to non-`SECURITY_DEPOSIT` obligations (`receivePayment`'s `obligationIdFilter`), so the money goes to rent and maintenance and the deposit stays owed. The amount is measured against the dues *excluding* the deposit, so a refusal states what is actually settleable. An unanswered column means the amount includes the deposit, as before.
...
- **Email is optional.** `createInvitation` requires a name, a phone and a room; the invitation goes out over WhatsApp to the phone. A blank email is accepted and two blanks are not the same person; an email that *is* given must be valid.
- **Where a row gets fixed depends on how many are wrong.** Up to eight rows — or a few in a large file — the review screen leads with in-place controls: each error carries the control for its own field, and re-checking runs the owner's typing through `revalidate`, the same validation as the file, on the same batch. Import is refused while changes are unchecked. **Beyond that, or when most of a smaller file is wrong**, it leads with the marked-up workbook instead, because forty single-field web edits with no fill-down or copy-paste is worse than the spreadsheet the owner already knows. Neither path is ever hidden; the other is offered underneath.
...
- **Rooms:** an inactive room blocks the row at preview (it would fail at confirm). A room with no base rent blocks the row only when the sheet gives no rent for that tenant either. **Sharing Type edits to an existing room are applied**, and Rooms-sheet issues name the owner's real spreadsheet line rather than a position in the parsed list.
...
- **A duplicate means a *live* tenancy, not any past one.** Matching uses the tenancy's own `phone_1`/`personal_email` as well as the linked profile, because `profile_id` is nullable and orphaned rows exist. Only `INVITED` and `ACTIVE` tenancies block an import (under [[Decisions#ADR-165|ADR-165]] a tenancy is live from invite, before acceptance). A `FORMER_TENANT`, `EXPIRED` or `CANCELLED` person can be imported again — matching every profile the owner ever had meant a returning tenant was blocked forever, though the single-tenant invite adopts them and the database already allows only one live tenancy per person.
- **Notes from the sheet become a `tenant_notes` row** against the imported tenant. Writing the note is never fatal: the tenancy already exists by that point, and losing a note must not fail the import (the failure is logged).
## The FOUNDING subscription plan ("Founding Partner") — auto-assigned to the first 10 owners (2026-09-10, [[Decisions#ADR-188|ADR-188]]; renamed and capacity-corrected 2026-09-12; onboarding-trigger + admin activation fixed 2026-09-12, [[Decisions#ADR-190|ADR-190]])

**FOUNDING (code `FOUNDING`, displayed to owners as "Founding Partner"; ₹2,000/month, 250 included beds, no upper bound on how many extra beds can be bought) is a launch offer for the first 10 owner accounts, and is never something an owner picks.** `subscriptionService.ensureForOwner` creates the owner's `owner_subscriptions` row (idempotent, one per owner) and, the first time it runs for that owner, claims a FOUNDING slot under a `pg_advisory_xact_lock` if `count(owner_subscriptions on FOUNDING) < 10`. Once all 10 slots are taken, new owners get the STARTER placeholder instead.

**2026-09-12 fix: the trigger is now the owner's first hostel (onboarding completion), not the Subscription page.** Previously `ensureForOwner` only ran lazily — the first time an owner (or admin) opened `GET /api/owner/subscription` or `/plans` — so Founding rank was really decided by who happened to open that page first, not by who signed up first, and an owner who never opened it stayed unclassified indefinitely. It is now called eagerly, best-effort, right after the owner's first hostel is created — the SAME existing milestone the lead-acquisition funnel already uses as "owner completed onboarding" (`leadInvitationService.markHostelCreated`, `platform_leads.status = HOSTEL_CREATED`) — from both hostel-creation entry points: `POST /api/owner/hostels` and `hostelProvisioningService.provision` (the wizard's one-shot path). The Subscription page's own `ensureForOwner` call remains, but purely as a defensive fallback for an owner who predates this fix or whose hostel-creation call failed — it must never be the primary mechanism. No special "Founding Partner" link or token exists or is needed: the normal onboarding/invitation flow every owner already goes through is what determines the slot.

**Admin "Mark as Paid & Activate" (2026-09-12).** Phase 1 has no payment gateway — the client pays ₹2,000 to Stayo entirely outside the app. `subscriptionPaymentService.markFoundingPaidAndActivate({ subscriptionId, adminId, reference? })` (`POST /api/platform-admin/subscriptions/[id]/activate-founding`, admin-only) lets an admin record that payment and approve it in one action instead of the general two-step record-cash-then-approve flow. Scoped to FOUNDING only (rejects `NOT_FOUNDING` otherwise) — the admin never types a plan code, price, included-beds count or expiry date; all of it is read from the owner's own subscription row and the canonical `subscription_plans` row. Internally it is just `recordCashPayment` immediately followed by `reviewPayment(APPROVE)` — the same reviewable-payment lifecycle every subscription payment goes through, so there is still no shortcut that activates a subscription without a payment record. `reviewPayment`'s own transaction remains the atomic boundary (payment → APPROVED, subscription → ACTIVE, one-month period via `computeBillingPeriod`'s calendar-month arithmetic, invoice issued). The admin subscriptions list/detail and the owner's own Subscription page now both show `founding_partner_number` (1–10, computed by `created_at` rank among current FOUNDING holders — not a stored column) wherever a Founding Partner is displayed.

**"Expiring soon" / "expired" owner messaging (2026-09-12).** No new subscription status was introduced — `PAUSED` (written by `runExpirySweep` when a paid period ends with no admin override) remains the canonical "expired" state, now labelled "Expired" on the owner's Subscription page with copy "Your subscription has expired. Please renew your subscription to continue using Stayo. Your data, tenants and properties are safe." An ACTIVE subscription within 7 days of `next_renewal_at` shows an "ending soon" warning (`deriveStatusView`, pure, date-derived — no new backend state) with copy "Your subscription is ending soon. Please renew your subscription to continue using Stayo."

**2026-09-12 correction: Founding Partner's active-tenant ceiling is real, not infinite.** It previously showed as bare "Unlimited" on the owner's usage view regardless of how many tenants were active, because `effectivePlanCapacity` short-circuited to `null` whenever `max_extra_beds` was `null`. That conflated two different things: "no upper bound on how many extra beds can be bought" (still true) and "the active-tenant ceiling itself never blocks" (was true, now false). The owner's own usage view now shows real, growing numbers — `0 / 250` for a fresh Founding Partner, growing to `260 / 260` etc. as extra beds are actually purchased at the same ₹10/bed mechanism every other plan uses — and `plan-capacity-service.assertCanActivate` genuinely blocks new tenant activation at the current ceiling, same as STARTER/GROWTH/PROFESSIONAL/PORTFOLIO. The only thing that remains unique to Founding Partner is that there is no maximum on how many extra beds can be bought (`max_extra_beds` stays `null` in the schema) — every other plan has a hard ceiling on extra-bed purchases (10/25/50/0). See [[Decisions#ADR-188|ADR-188]] for the schema/code detail. Verified against the live database at the time of this change: exactly one owner held Founding Partner, `PAUSED`, 1 active tenant against the 250 ceiling — zero real-world impact from the correction.

- "First 10" counts **owners**, not tenants or hostels. A slot is held by an `owner_subscriptions` row whose `plan_id` **or** `pending_plan_id` is FOUNDING.
- Assignment is at **`owner_subscriptions` row creation time** (first hostel created, see the 2026-09-12 fix above), so a new owner holds a slot from onboarding completion, well before paying. Admin can reassign via `change-plan`.
- FOUNDING owners still go through the full `PENDING_PAYMENT` → submit ₹2,000 → admin `APPROVED` flow. Only the pre-assigned plan differs from an ordinary owner.
- `GET /api/owner/subscription/plans` returns **only** public plans (Starter / Growth / Professional / Portfolio) and never FOUNDING. The owner billing page shows a FOUNDING owner a fixed "Your plan" card + payment step with no plan picker — a FOUNDING owner never sees the normal-plan picker (`OwnerSubscriptionPage.showPlanPicker` is forced false whenever `sub.plan.code === 'FOUNDING'`).
- Backfill: `apps/backend/scripts/backfill-founding-owners.ts` (dry-run default, `--apply` to write) moves pre-existing `PENDING_PAYMENT` owners still on the STARTER placeholder — with no submitted/approved payment — onto FOUNDING, oldest first, while slots remain. **Not applied in this change** — see [[Changelog]].

## Included beds + paid extra beds — the plan capacity model (2026-09-10)

Every plan's active-tenant ceiling is now **`included_beds` + the owner's own paid `extra_beds`** (never the plan's theoretical maximum). This replaced the old single `capacity_max`-only model; `capacity_max` is kept in sync (`included_beds + max_extra_beds`, or `NULL` when uncapped) purely for display/legacy readers — the actual ceiling enforced per owner is `plan-capacity-service.effectivePlanCapacity(plan, owner_subscriptions.extra_beds)`.

| Plan | Included beds | Max paid extra beds | Extra-bed price |
|---|---|---|---|
| FOUNDING ("Founding Partner") | 250 | **No cap on how many can be bought** (`max_extra_beds: null`) — the active-tenant ceiling itself is still real and grows only as beds are actually purchased, same enforcement as every other plan | ₹10/bed |
| STARTER | 50 | 10 | ₹10/bed |
| GROWTH | 100 | 25 | ₹10/bed |
| PROFESSIONAL | 250 | 50 | ₹10/bed |
| PORTFOLIO | 500 | **0 — not offered** (`max_extra_beds: 0`, `extra_bed_price_paise: null`) | — |

- **Portfolio's extra-bed allowance is an explicit unresolved business value, not a guessed number.** `max_extra_beds: 0` on Portfolio means "extra beds are not purchasable on this plan yet" — distinct from "you've hit your allowance, upgrade" (the message `validateExtraBeds` returns is different for each case). Portfolio's base `capacity_max` (500) is preserved unchanged. See `scripts/seed-subscription-plans.ts` and the Portfolio row in `Database.md` for where this is enforced; do not invent a Portfolio extra-bed number without a business decision.
- **Extra-bed validation is server-side only**, in `subscription-rules.ts#validateExtraBeds` — called on submit (`subscriptionPaymentService.createSubmittedPayment`), again at approval (`reviewPayment`, since the plan's allowance could have changed between submit and approval), and in the admin's immediate `change-plan`. The frontend never decides whether a requested extra-bed count is allowed; it only renders the allowance the backend already returned.
- If an owner needs more capacity than their plan's `included_beds + max_extra_beds` allows, the only path is **upgrading to the next plan** — there is no override to raise a plan's extra-bed ceiling for one owner (the admin's "extend" override is date-based access, not a capacity override).
- A same-plan top-up of extra beds (more beds, same plan) is its own `classifyPlanChange` outcome, `EXTRA_BEDS` — applied immediately like an upgrade, billing period and renewal date untouched; only the *incremental* new beds are charged in full (not the whole new total).
- Admin subscription list/detail and the owner's own billing page both display `included_beds` vs `max_extra_beds` (`planLabel`, `subscriptionAdminView.ts`) rather than only the legacy `capacity_max` range, so "50 included, +10 extra" is visible instead of an opaque "1–60".

### Extra beds are a RECURRING monthly charge, and their amount is always server-computed (Phase 6.6, 2026-09-10, [[Decisions#ADR-189|ADR-189]])

Paid extra beds are not a one-off purchase — they recur every billing period alongside the plan price, for as long as `owner_subscriptions.extra_beds` stays nonzero. What "recurring" means concretely, per payment kind (`classifyPlanChange`):

- **`NEW` / `RENEWAL`** (a fresh activation, or renewing the same/queued-downgrade plan): the invoice charges the plan's full price **plus `extra_beds × extra_bed_price_paise` on the FULL current total**, every period — this is the actual recurring bill. `computeRenewal` itself stays pure/plan-price-only; the extra-bed amount is added on top in `subscription-payment-service.ts#planApprovalOutcome`.
- **`EXTRA_BEDS`** (same-plan top-up, more beds mid-period): a ONE-TIME charge for only the **incremental** new beds (`(new_total − current) × price`) — the full new total becomes the recurring base for the *next* renewal, but this particular invoice doesn't re-charge beds already paid for this period.
- **`UPGRADE`**: the extra-bed portion is charged in full on the requested new total (never prorated, unlike the plan-price delta, which is prorated as before).
- **`DOWNGRADE`** (the scheduling payment, typically ₹0): carries no extra-bed component — the queued plan's extra beds are whatever the *later renewal* payment requests.

**The extra-bed amount is never trusted from a payment's claimed `amount_paise`** — it is always `(the relevant quantity above) × plan.extra_bed_price_paise`, both server data. For a manual (no-gateway) payment where the admin/owner types a total, an inflated or mistaken total lands entirely and visibly in the **plan** portion (see below), never silently hidden inside the extra-bed figure.

**Invoices itemize the charge, snapshotted, not re-derived.** `subscription_invoices` carries `plan_amount_paise`, `extra_bed_unit_price_paise` (a snapshot of the per-bed price *at invoice creation* — never re-read from the live plan later, so a subsequent admin price edit can't retroactively change what an already-issued invoice says it charged), and `extra_bed_amount_paise`, with `plan_amount_paise + extra_bed_amount_paise + tax_paise (always 0) = amount_paise` enforced in `subscription-invoice-service.ts#createInvoiceForApprovedPayment` (throws if a caller's breakdown doesn't reconcile). The invoice PDF (`lib/pdf/subscription-invoice-content.ts`) shows Plan / Extra beds / Subtotal / Tax / Total as separate summary rows when extra beds are present.

**A plan change that would leave an existing extra-bed count incompatible with the target plan is refused up front — never silently applied, never auto-reduced.** Both the owner's `subscriptionDowngradeService.scheduleDowngrade` and the admin's `subscriptionAdminService.changePlan` (`NEXT_PERIOD` branch) validate the subscription's *current* `extra_beds` against the target plan's `max_extra_beds` before queuing `pending_plan_id`, and throw `409 EXTRA_BEDS_EXCEEDS_TARGET_PLAN` if it doesn't fit (e.g. a PROFESSIONAL owner with 30 extra beds cannot schedule a downgrade to GROWTH, whose ceiling is 25, without first reducing). There is deliberately no automatic reduction — this is a "resolve it first" refusal, not a silent-discard shortcut.

**The owner billing form carries the current extra-bed count FORWARD on a genuine upgrade** (`initialExtraBedsForSelection`, `subscriptionView.ts`) rather than resetting it to 0 — resetting would have silently dropped already-paid recurring beds the moment the owner picked a higher plan. Every other plan-selection case (a fresh `NEW`/`RENEWAL` choice, `DOWNGRADE`) still starts the field at 0. The backend independently validates whatever the owner ends up submitting and refuses (`EXTRA_BEDS_LIMIT_EXCEEDED`) rather than discards an incompatible carried-forward count — e.g. "upgrading" by price from PROFESSIONAL(30 extra) into PORTFOLIO (which offers none) is rejected at approval, not silently landed with `extra_beds` reset to 0.

## Payment-amount mismatch is surfaced, never enforced (Phase 6.9, 2026-09-10)

A manual (no-gateway) payment's declared `amount_paise` is never authoritative for the invoice — this was already true since Phase 6.6 (see above). Phase 6.9 adds a purely **additive, non-blocking** admin-visible signal when the declared amount doesn't match what the server would calculate (`subscription-rules.ts#computeExpectedTotalPaise`, the same per-`PlanChangeKind` formula the approval path already uses to derive the invoice). Rules:

- **Never rejects the payment.** A mismatch is informational only — the admin still decides whether to approve based on the actual proof (bank/UPI screenshot), same as always.
- **Never changes the invoice.** The invoice's `plan_amount_paise`/`extra_bed_amount_paise` were already 100% server-derived before this phase; the mismatch check doesn't touch that path.
- **Visible in two places:** the admin payment-review queue (`GET /api/platform-admin/subscription-payments` → `expected_amount_paise`, `amount_mismatch` per row, shown as a `⚠ expected ₹X` line in the admin UI) and the `SUBSCRIPTION_PAYMENT_APPROVED` audit event (captured once, at the moment of approval — the permanent historical record). No proof/transaction-reference/UPI data is added to either surface.
- `getExpectedAmount` is a **live** preview against the subscription's *current* state — re-running it against an already-decided historical payment can disagree with what was true when it was actually approved (the subscription has since moved on). The audit event's captured `expected_amount_paise` is the trustworthy historical value, not a later re-query.

## RLS on the four owner-level billing tables (Phase 6.9, 2026-09-10, [[Decisions#ADR-189|ADR-189]])

`owner_subscriptions`, `subscription_payments`, `subscription_invoices`, `owner_billing_profiles` had Row Level Security **disabled** since Phase 1 — flagged as a security gap in Phase 6.7's pre-flight audit, resolved in Phase 6.9 after tracing every access path (not assumed safe):

- The backend's two DB connections (`lib/db.ts`'s `prisma`, via the pooled Postgres superuser role; and `supabase`, via `SUPABASE_SERVICE_ROLE_KEY`) both implicitly bypass RLS — enabling it changes nothing about how the application itself reads or writes these tables.
