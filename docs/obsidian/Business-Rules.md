...
- **Small corrections are made in the review screen, not in Excel.** Each error carries the control for its own field, and re-checking runs the owner's typing through `revalidate` — the same validation as the file, on the same batch. Import is refused while changes are unchecked.
