...
**An owner may be a tenant of a *different* hostel; never of their own (2026-09-01, [[Decisions#ADR-158|ADR-158]]).** Whether the matched profile is allowed through the checks above is not "is this profile role TENANT" but "does this profile own *this specific hostel*" — `tenancy-claim-service.ts` and `owner-managed-tenancy-service.ts` compare the matched profile's id against this tenancy's actual `owner_id`, not against a blanket non-TENANT-role refusal. An `OWNER` of Hostel A may claim/be adopted into a tenancy at Hostel B; an `OWNER` of Hostel A may never claim/be adopted into a tenancy at Hostel A itself. An `ADMIN` profile, or an `OWNER` who does own this exact hostel, is still refused. **Known limitation:** `profile.role` remains a single global field, and login session role is derived from it — so an owner who successfully becomes a different hostel's tenant still authenticates with an `OWNER` session today, and cannot yet reach that tenancy's portal via `resolveTenantScope`. This ADR only removed the incorrect rejection; making the resulting account usable from both sides is separate, tracked work — see [[TODO]].

## Lead creation cannot re-capture an existing tenant, and an owner cannot invite themselves (2026-09-01, [[Decisions#ADR-158|ADR-158]])
...
| **Claim** *(Phase 2, 2026-08-27)* | Tenant OTP-verifies their number (`POST /api/tenancy-claim/confirm`) | `OWNER_MANAGED → SELF_SERVE`, profile linked (reused if adoption already created one) or created, same `tenant_id`, same financial history. See the claim section below and [[APIs]]. |
...
**What makes a tenancy claimable — `isClaimable`:** `access_mode === "OWNER_MANAGED"` **and** `status === "ACTIVE"` — plus, since 2026-08-28, a login check on the bound profile (see immediately below).
...
**Revised claimability rule (2026-08-28, [[Decisions#ADR-136|ADR-136]]): a bound profile is claimable if it cannot sign in.** `isClaimable` originally read `profile_id == null` as "unclaimed" and any non-null `profile_id` as "already claimed, refuse." Once adoption started linking a profile unconditionally (see "Owner-managed tenants" above), every `OWNER_MANAGED` tenancy has a non-null `profile_id` — so the original rule would have refused every claim outright, breaking the feature this same section documents. `isClaimable` now takes an extra input, `profile_has_login` (`profiles.auth_user_id != null`, computed in `tenancy-claim-service.ts`'s `withLoginFlag` at both call sites — `listClaimable` and `confirm`): a bound profile with **no** login is still claimable (that login-less shell *is* the owner holding the account, not a claim already made), a bound profile that **can** sign in is not (claiming it would be a takeover of a real account). **A missing/undefined `profile_has_login` fails closed** — treated as "has a login," so refused — so a caller that forgets to select `profiles.auth_user_id` cannot accidentally make every bound tenancy claimable.
