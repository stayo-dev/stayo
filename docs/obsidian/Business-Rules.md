...
| **Claim** *(Phase 2 — not built)* | Tenant OTP-verifies their number | `OWNER_MANAGED → SELF_SERVE`, profile created/linked, same `tenant_id`, same financial history. See [[TODO]]. |
...
**Not yet built (Phase 2):** the OTP-gated claim flow, the pre-link confirmation card, the multi-hostel/multi-match picker, and Revoke (`SELF_SERVE → OWNER_MANAGED` for a tenant who stopped using the app). See [[TODO]].
