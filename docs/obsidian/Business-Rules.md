...
- **Amount already paid** (parsed from an *Amount Already Paid* column; the current CSV template does not offer one yet) is settled inside the invitation transaction by `financialPaymentFacade.receivePayment`, **FIFO across every due** — back-rent, maintenance and the deposit obligation. A payment method is required: a row with an amount and no method is blocked at preview (`PAYMENT_METHOD_MISSING`) instead of failing at execution. **An amount above what the tenant owes is now flagged at preview too** (`OVERPAID`, `NEEDS_CHOICE`) — `financial-plan.ts` composes `buildInviteSettlementPreview`, the same pure planner behind the single-invite settlement panel, rather than recalculating dues.
