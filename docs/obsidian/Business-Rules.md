...
## The FOUNDING subscription plan — auto-assigned to the first 10 owners (2026-09-10, [[Decisions#ADR-188|ADR-188]])

**FOUNDING (₹2,000/month, 250 included beds, unlimited paid extra beds) is a launch offer for the first 10 owner accounts, and is never something an owner picks.** When an owner's `owner_subscriptions` row is first created (`subscriptionService.ensureForOwner`, on first visit to any subscription surface), the service claims a FOUNDING slot under a `pg_advisory_xact_lock` if `count(owner_subscriptions on FOUNDING) < 10`; that owner's `plan_id` is set to FOUNDING. Once all 10 slots are taken, new owners get the STARTER placeholder instead.
...
- Assignment is at **creation time**, so a new owner holds a slot from first login even before paying. Admin can reassign via `change-plan`.
...
|---|---|---|---|
| FOUNDING | 250 | **No ceiling** (`max_extra_beds: null`) | ₹10/bed |
