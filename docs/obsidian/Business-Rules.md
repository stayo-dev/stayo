...
7. ~~Tenant activation's phone-OTP requirement is deliberately unchanged.~~ **Superseded 2026-08-24 ([[Decisions#ADR-096|ADR-096]]):** the primary-phone OTP requirement was removed from tenant activation entirely (for every invitee, not just lead-accepted ones) — see the new section below.

See [[APIs]], [[Database]], [[Features]], [[Decisions]].

## Tenant Activation — primary phone is contact information, not an auth factor (2026-08-24)

Added [[Decisions#ADR-096|ADR-096]], reversing [[Decisions#ADR-087|ADR-087]] point 7. **Files:** `apps/backend/src/services/tenants/activation-workflow-service.ts` (`saveAccount`), `apps/frontend/src/platforms/tenant/onboarding/ActivationPage.tsx`, `.../steps/WelcomeIdentityStep.tsx`.

1. **The tenant's primary phone number is owner-supplied contact information, not something the tenant must prove ownership of at activation.** The owner already captures name, email, and phone when creating the invitation (Add Tenant wizard, or lead-accept → convert-to-invitation, see the Admissions/Leads section above); activation's ACCOUNT step no longer re-verifies that phone via OTP. The field is still shown, still pre-filled from the invitation, and still editable — it's just no longer gated on receiving/entering a code.
2. **`mobile_verified`/`phone_verified` are still written `true` on ACCOUNT-step completion** — this is bookkeeping for the activation state machine (`assertTransition()`'s `account_setup_completed` check, which blocks every later step until this is true), not a claim that phone ownership was proven. No login, dashboard, WhatsApp, or payment code reads these fields for tenants — the only consumer that mattered was already-known to not require real verification (`tenant-invitation-lifecycle-service.ts`'s `startActivation()` set them unconditionally, with no OTP, for first-time invitees, even before this change).
3. **Guardian-phone OTP (`purpose: "ParentVerify"`, the PROFILE step) is unaffected and still required.** A guardian is a third party with no owner-vouching or prior platform relationship, so proving they hold their own phone remains load-bearing there — this rule applies only to the tenant's own primary phone.
4. **This is broader than the lead-accepted-tenant case ADR-087 point 7 considered** — it removes the OTP requirement for *every* tenant activation, not just ones that came through the enquiry/lead funnel, since the underlying reasoning (owner already supplied and vouches for the number) applies equally to a directly-invited tenant.
5. **Self-serve owner/lead-capture phone-OTP is unchanged** (`/api/auth/owner-signup`, `/api/leads/self-serve`) — those gate a signup where the phone is the only thing tying an anonymous request to a real person before any account exists; an invited tenant's activation starts from an owner-vetted invitation, which is a materially different trust position.

See [[Decisions#ADR-096|ADR-096]], [[APIs]], [[Changelog]].
