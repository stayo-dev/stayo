...
**A second, phone-keyed version of the same guarantee was added 2026-08-28 ([[Decisions#ADR-135|ADR-135]]).** The profile-keyed index above only ever constrains rows that carry a `profile_id` — a tenancy created with `profile_id: NULL` (as owner-managed adoption used to, see below) sailed straight past it. Migration `20260827180000_one_live_tenancy_per_phone` adds a second partial unique index, on `tenants.phone_1` for `ACTIVE`/`INVITED` rows, independent of whether `profile_id` is set at all. **Not yet applied to any database** — see [[Database]] and [[TODO]] for the unapplied-migration status and the query that finds violating rows.
...
| **Keep the records myself meanwhile** *(renamed 2026-08-28, [[Decisions#ADR-135|ADR-135]])* | Owner, on the success screen right after sending an invite (`InviteDeliveryResult`) | Same `adopt` call as above, against the tenancy `inviteMutation` just created — no second tenancy, no suppressed-notification flag. **Replaces** the earlier pre-send "Just add to my records" exit, which created its own tenancy via `createInvitation({ suppressInvitationNotification: true })` before adopting it; that exit no longer exists. |
