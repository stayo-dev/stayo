...
- **Resident & guardian commands** — see the dedicated section below. Superseded 2026-08-27 by [[Decisions#ADR-128|ADR-128]]: the old `BAL`/`BALANCE`/`SWITCH`/`DUES`/`PAY`/`STATUS`/`HELP` surface is replaced by `RENT`, `PAY`, `PLAN`, `RECEIPT`, `HELP`.
