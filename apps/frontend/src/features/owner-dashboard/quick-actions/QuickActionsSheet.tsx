...
/** Home FAB's Quick Actions menu, per Stayo App.dc.html's 3-item modal — Create Food Poll added as a 4th, per explicit request. */
    ...
    <BottomSheet open={open} onOpenChange={(v) => !v && onClose()} title="Quick actions">
      <div className="flex flex-col gap-2">
        ...
        <button type="button" onClick={onCreateFoodPoll} className={rowCls}>
          ...
          <span className="font-display text-[12.5px] font-bold text-foreground">Create Food Poll</span>
        </button>
      </div>
    </BottomSheet>
  );
