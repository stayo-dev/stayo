...
import { ALERT_CATEGORY_LABELS, type AlertCategory } from '@shared/mocks/alerts';
import { useAlerts } from '../hooks/useAlerts';

const CATEGORIES: AlertCategory[] = ['leads', 'admin', 'renewals', 'requests'];
...
/** Alerts tab — Leads/Admin/Renewals/Requests, per Stayo App.dc.html. Reached via Home's bell icon, not the bottom nav (design has no Alerts nav icon). */
      ...
      <div className="flex flex-col gap-2">
        {alerts.category === 'leads' &&
          (alerts.leads.length === 0 ? (
            <EmptyState icon={<Inbox className="h-5 w-5" />} title="No leads yet" />
          ) : (
            alerts.leads.map((l) => (
              <div key={l.id} className={rowCard} onClick={() => alerts.markRead('leads', l.id)}>
                <div className="flex items-center gap-3">
                  {!l.read && <span className="h-2 w-2 flex-none rounded-full bg-info" />}
                  <span className="flex h-10 w-10 flex-none items-center justify-center rounded-full bg-info/10 font-display text-[13px] font-bold text-info">
                    {initials(l.name)}
                  </span>
                  ...
                  <span className="flex-none rounded-md bg-info/10 px-2 py-0.5 text-[10.5px] font-semibold text-info">{l.stage}</span>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={soon} className={actionBtn}>Follow up</button>
