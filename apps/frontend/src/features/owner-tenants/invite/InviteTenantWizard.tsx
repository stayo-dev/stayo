...
export function InviteTenantWizard({ open, onClose, initialData, leadId }: InviteTenantWizardProps) {
  ...
  return (
    ...
    >
      ...
      {wizard.submitConflict || wizard.ownerManagedConflict ? (
        <div className="mb-4 rounded-xl border border-destructive/25 bg-destructive/10 px-3.5 py-2.5 text-[12.5px] font-semibold text-destructive">
          <p className="mb-0.5 font-bold">{(wizard.submitConflict || wizard.ownerManagedConflict)?.title}</p>
          <p className="font-medium">{(wizard.submitConflict || wizard.ownerManagedConflict)?.body}</p>
        </div>
