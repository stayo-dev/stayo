...
export function buildServiceRequestFormConfigs(ctx: FormContext): Record<string, FormConfig> {
  ...
  const reportBugOptions = {
    pay: 'Payments',
    foodorder: 'Food ordering',
    roomsvc: 'Room services',
    notif: 'Notifications',
  ...
  return {
    raise_ticket: {
      ...
      sub: 'Get help from the hostel team',
      ...
      successSub: 'Our team will get back to you soon. Track it anytime from your profile.',
      refPrefix: 'T',
    ...
    report_bug: {
      ...
      hasIcon: true,
      options: [
        { id: 'pay', label: 'Payments', icon: icon(CreditCard) },
        { id: 'foodorder', label: 'Food ordering', icon: icon(UtensilsCrossed) },
        { id: 'roomsvc', label: 'Room services', icon: icon(DoorOpen) },
        { id: 'notif', label: 'Notifications', icon: icon(Bell) },
        { id: 'other', label: 'Somewhere else', icon: icon(CircleHelp) },
      ],
      photos: true,
      banner: 'Add a screenshot and the steps to reproduce — it helps us fix it faster.',
      note: { label: 'What went wrong?', placeholder: 'What you did, what you expected, what happened…' },
      submitLabel: 'Submit bug report',
      successTitle: 'Bug reported',
      successSub: 'Thanks for the report! Our team will investigate and update you here.',
      refPrefix: 'B',
      onSubmit: submitWithCategory('MAINTENANCE', reportBugOptions),
    },
    svc_room_change: {
