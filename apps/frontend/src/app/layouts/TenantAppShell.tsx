 ...
 * Room / Food / Profile). Flatter IA than the owner app (no drill-down), one
 * modal type (pay sheet) — this shell is just the persistent nav + content
 * area, matching that simplicity. No header/logo bar — the mockup has none
 * (verified 2026-07-27 fidelity audit; an earlier pass added one with no
 * design basis, since removed).
 */
const TENANT_TABS = [
...
export function TenantAppShell() {
  return (
    <ThemeProvider theme="product">
      <div className="flex min-h-screen flex-col bg-background text-foreground [background-image:linear-gradient(#EBDCCF_1px,transparent_1px),linear-gradient(90deg,#EBDCCF_1px,transparent_1px)] [background-size:52px_52px]">
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card pb-[env(safe-area-inset-bottom,0px)] shadow-[0_-4px_16px_rgba(40,30,20,0.03)]">
