...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cacheKey = redisKeys.analytics.operations(scope.owner_id, hostelId, rangeHash);
    const cached = await getCachedDashboard(cacheKey);
    if (cached) return apiResponse(cached);

    const data = await timed(
      "analytics.operations",
      () => analyticsService.getOperationsDashboard(scope.owner_id, start, end, hostelId),
    ...
    return apiError(error.message || "Failed to fetch operations data");
  }
}
