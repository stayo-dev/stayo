...
import {
  ...
  isAgreementRequired,
  nextActivationStep,
  requiredActivationSteps, isGuardianSignatureRequired, validateAgreementSignatures } from "./agreement-requirement";
...
export class ActivationWorkflowService {
  ...
  /**
   * Whether this hostel requires a parent/guardian co-signature.
   *
   * Fetched explicitly by hostel id, like its sibling above. The resolved
   * tenant does not include `hostels`, so reading the policy off it would
   * silently evaluate to "not required" and the setting would never work.
   */
